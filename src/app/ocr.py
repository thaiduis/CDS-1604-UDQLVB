...
# Optional: EasyOCR for improved accuracy on images
EASYOCR_AVAILABLE = False
EASYOCR_GPU_ENABLED = os.getenv("EASYOCR_GPU", "false").strip().lower() in {"1", "true", "yes", "on"}
try:
...
# Build a lightweight Vietnamese vocabulary for autocorrect
VN_BASE_VOCAB = set(VIETNAMESE_WORDS + [
    'văn', 'bản', 'người', 'chúng', 'ta', 'muốn', 'không', 'phải', 'những', 'nhưng', 'điều', 'gì',
    'cuộc', 'sống', 'học', 'tập', 'công', 'việc', 'thời', 'gian', 'quan', 'tâm', 'vấn', 'đề',
    'giải', 'quyết', 'hiện', 'tại', 'bởi', 'vì', 'nếu', 'cần', 'thiết', 'thật', 'sự', 'rất', 'nhiều',
    'trẻ', 'em', 'thanh', 'niên', 'người', 'lớn', 'gia', 'đình', 'bạn', 'bè', 'thầy', 'cô', 'trường',
])
...
def auto_correct_vietnamese_preserve_structure(text: str) -> str:
    """
    Simple dictionary + edit-distance based autocorrect for Vietnamese.
    - Preserves line breaks and punctuation.
    - Uses unaccented comparison to better map missing diacritics.
    """
    if not text:
        return text
    lines = text.split('\n')
    ...
    for line in lines:
        ...
        for tok in tokens:
            if not tok.isalpha() or len(tok) < 3:
                ...
                continue
            base = tok.lower()
            if base in VN_BASE_VOCAB:
                out_tokens.append(tok)
                continue
            # Compare unaccented forms
            base_na = _strip_accents(base)
            best = base
            best_d = 10**9
            for cand in VN_BASE_VOCAB:
                d = levenshtein_distance(base_na, _strip_accents(cand))
                if d < best_d and d <= max(1, len(base) // 3):
                    best_d = d
                    best = cand
            if best != base:
                out_tokens.append(_preserve_case(tok, best))
            else:
                out_tokens.append(tok)
...
def extract_text_from_image(image_path: str, lang: str = "vie+eng", fast_mode: bool = True) -> str:
	"""
	Enhanced image OCR with optional EasyOCR first, then Tesseract fallback.
	"""
	logger.info(f"Processing image: {image_path}")
	
	try:
		# 0) Try EasyOCR first if available (often better for natural images)
		if EASYOCR_AVAILABLE:
			try:
				langs = []
				# Map Tesseract-style lang to EasyOCR codes
				if 'vie' in lang or 'vi' in lang:
					langs.append('vi')
				if 'eng' in lang or 'en' in lang or not langs:
					langs.append('en')
				logger.info(f"EasyOCR trying with languages: {langs}")
				# Try GPU if requested, fallback to CPU if not available
				reader = None
				if EASYOCR_GPU_ENABLED:
					try:
						reader = easyocr.Reader(langs, gpu=True)
						logger.info("EasyOCR initialized with GPU=True")
				...
				for item in results:
					if isinstance(item, (list, tuple)) and len(item) >= 3:
						...
						if isinstance(txt, str) and txt.strip():
							easy_text_parts.append(txt.strip())
						try:
							confidences.append(float(conf))
						except Exception:
							pass
				easy_text = "\n".join(easy_text_parts).strip()
				avg_easy_conf = (sum(confidences) / len(confidences)) if confidences else 0.0
				logger.info(f"EasyOCR extracted {len(easy_text)} chars, avg conf ~ {avg_easy_conf:.2f}")
				if easy_text and (avg_easy_conf >= 0.5 or len(easy_text) >= 50):
					return correct_vietnamese_text_preserve_structure(easy_text)
			except Exception as e:
				logger.warning(f"EasyOCR failed or not suitable: {e}")
		...
		# Try with best PSM configs on the original image
		configs = [
			'--psm 6',	# Single block
			'--psm 3',	# Single column
			'--psm 8',	# Single word
		...
		# If still weak, try preprocessing variants and Tesseract again
		if best_confidence < 60:
			logger.info("Trying Tesseract with preprocessed variants...")
			variants = preprocess_image_fast(image_path) if fast_mode else preprocess_image(image_path)
			for idx, variant in enumerate(variants):
				try:
					data = pytesseract.image_to_data(variant, lang=lang, config='--psm 6', output_type=pytesseract.Output.DICT)
					confs = [int(c) for c in data['conf'] if str(c).isdigit() and int(c) > 0]
					avg = sum(confs) / len(confs) if confs else 0
					text = pytesseract.image_to_string(variant, lang=lang, config='--psm 6')
					if avg > best_confidence and text.strip():
						best_confidence = avg
						best_text = text
						logger.info(f"Variant {idx} improved conf to {avg:.1f}")
				except Exception as e:
					logger.debug(f"Variant {idx} OCR failed: {e}")
